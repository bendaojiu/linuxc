...
{
  sql_init();
  connection = mysql_real_connect(&mysql, Hostname, User, Password, DBName,
      0, 0, 0);
  if (connection == NULL)
  {
    printf("%s\n", mysql_error(&mysql));
...
void addurl(const char *url, const char *name, const char *description, char **buf)
{
  ...
  sprintf(content, "<a href=\"https://%s\">%s</a></br>%s%s</br></br>", url, name,
      name, description);

  if (*buf != NULL)//addurl函数已经调用了
  {
  ...
  {
    int contentlen = strlen(content);//得到content中字符串的长度
    *buf = malloc(content+1);//根据content中字符串的长度动态分配内存空间buf
...
int query_result(const char *name, char **buf)
{
  ...
  sprintf(sSQL, "select url, name, description from baidu where name like '%%%s%%'", name);
  printf("%s\n", sSQL);
